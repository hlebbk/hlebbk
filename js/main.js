// main.js — 100% рабочий, декабрь 2025
    ...
    hits.forEach(item => {
        ...
        container.innerHTML += `
            <div class="hit-card">
                ...
                <div class="hit-info"><h3>${item.name}</h3><p class="hit-price">${item.price} ₽</p>${stats[item.name]>0?'<span class="popular-tag">Хит!</span>':''}</div>
            </div>`;
    });
}
...
function initReviewsWithTelegram() {
    ...
    form.onsubmit = e => {
        ...
        if (!name || !text) return alert('Заполните имя и текст!');
    ...
    list.innerHTML = published.length === 0
        ...
        : published.map(r => `
            <div style="background:#fff;padding:20px;margin:15px 0;border-radius:10px;box-shadow:0 3px 8px rgba(0,0,0,0.1);">
